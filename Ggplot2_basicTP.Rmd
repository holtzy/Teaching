...
<br><br><span class="questionNumber">Q0.1</span> - Install and load `dplyr`, a package that will be useful all along the practical.
...
<br><br><span class="questionNumber">Q1.1</span> - Load the `gapminder` dataset stored in the `gapminder` package. Have a look to the 6 first rows using the `head()` function. Describe briefly what you see as comments in your script. Check how many rows are available with `nrow()`
...
<br><br><span class="questionNumber">Q1.2</span> - How many years are available in this dataset? How many data-points for each year? Full code is provided for this question.
...
<br><br><span class="questionNumber">Q1.4</span> On the previous chart, one country is very different. Which one is it?
```{r, fig.align="center", eval=FALSE, class.source="Question",echo=TRUE }
# Number of different year?
gapminder %>%
  filter(...) 
...
<br><br><span class="questionNumber">Q1.5</span> Build the same chart, but get rid of this country. What trend do you observe? Does it make sense? What's missing? What could be better?
```{r, fig.align="center", eval=FALSE, class.source="Question",echo=TRUE }
# basic scatterplot
gapminder %>%
  filter(...) %>%
...
<br><br><span class="questionNumber">Q1.6</span> - Color dots according to their `continent`. In the `aes()` part of the code, use the `color` argument.
```{r, fig.align="center", eval=FALSE, class.source="Question",echo=TRUE }
gapminder %>%
  filter(...) %>%
  ggplot( aes(..., color= ...))  +
...
<br><br><span class="questionNumber">Q1.7</span> - Let's observe an additional variable: make the circle size proportionnal to the population (`pop`). This is done with the `size` argument of `aes()`. How do you call this kind of chart?
...
<br><br><span class="questionNumber">Bonus</span> You're in advance? Try to do the following:
...
<br><br><span class="questionNumber">Q2.1.1</span> - How many rows in the dataset? (use `nrow()`) What is the min? The max? (use `summary()`). Do you see anything strange? What kind of chart would you build to visualize this kind of data?
```{r}
#nrow(data)
#summary(data)
```

<br><br><span class="questionNumber">Q2.1.2</span> - Build a histogram of the data with `geom_histogram()`. Are you happy with the output? How can we improve it?
```{r, fig.align="center"}
data %>%
  ggplot( aes(x=price)) +
    geom_histogram()
...
<br><br><span class="questionNumber">Q2.1.3</span> - Build a histogram without price over 1500 euros. `ggplot2` displays a warning message, why? What does it mean? What's the main caveat of histograms?
```{r, fig.align="center"}
data %>%
  filter(price<1500) %>%
  ggplot( aes(x=price)) +
...
<br><br><span class="questionNumber">Q2.1.3</span> - Build the histogram with different values of `binwidth`, for prices <400. What do you observe? Is it important to play with this parameter?
```{r, fig.align="center"}
data %>%
  filter(price<400) %>%
  ggplot( aes(x=price)) +
...
<br><br><span class="questionNumber">Q2.1.4</span> - Use `geom_density()` to build a density chart. Use the `fill` argument to set the color. Use the `help()` function to find out what is the equivalent of `bin_width` for density chart? Check its effect using different values.
...
<br><br><span class="questionNumber">Q2.2.1</span> - As usual, check data main features with `nrow()`, `summary()` or any other function you think is useful.
...
<br><br><span class="questionNumber">Q2.2.2</span> - What kind of chart would allow to compare the 8 categories?


<br><br><span class="questionNumber">Q2.2.3</span> - Build a basic boxplot using the default options of `geom_boxplot`()
...
<br><br><span class="questionNumber">Q2.2.4</span> - What do you observe? Can you improve this chart? What would you change? Do you remind what the different parts of the box mean?

<br><br><span class="questionNumber">Q2.2.5</span> - Apply the following modifications to the previous boxplot:
...
<br><br><span class="questionNumber">Q2.2.6</span> - What is the main caveat with boxplot? How can we do better?
...
<br><br><span class="questionNumber">Q3.1</span> - What kind of chart can you build with this dataset? Which one would be the best in your opinion?

<br><br><span class="questionNumber">Q3.2</span> - Start with a basic barplot using `geom_bar()`.
```{r, fig.align="center"}
data %>%
...
<br><br><span class="questionNumber">Q3.3</span> - Previous barplot is a bit disappointing isn't it? What can you improve? Do it!
